...
//----------------------------------------------------------------------------
  ...
  // void Panel_ILI9331::setBrightness(uint8_t brightness)
  // {
  //   startWrite();
  //   writeCommand(0xB1, 1);
  //   _bus->writeData(brightness << 8, 16);
  //   endWrite();
  // }

  void Panel_ILI9331::setInvert(bool invert)
  {
    _invert = invert;
/*
    startWrite();
    write_command(CMD_DISPLAY_CTRL1);
...
//*/
  ...
  void Panel_ILI9331::setSleep(bool flg)
  {
    startWrite();
    write_command(CMD_POWER_CTRL1);
  ...
  void Panel_ILI9331::setPowerSave(bool flg)
  {
    startWrite();
    write_command(CMD_POWER_CTRL1);
    uint_fast16_t data = flg 
  ...
  void Panel_ILI9331::writeCommand(uint32_t data, uint_fast8_t length)
  {
    if (_has_align_data)
    {
  ...
  void Panel_ILI9331::setWindow(uint_fast16_t xs, uint_fast16_t ys, uint_fast16_t xe, uint_fast16_t ye)
  {
    set_window(xs, ys, xe, ye, CMD_RAMWR);
  }

  void Panel_ILI9331::drawPixelPreclipped(uint_fast16_t x, uint_fast16_t y, uint32_t rawcolor)
  {
  ...
  void Panel_ILI9331::writeFillRectPreclipped(uint_fast16_t x, uint_fast16_t y, uint_fast16_t w, uint_fast16_t h, uint32_t rawcolor)
  {
    uint32_t len = w * h;
  ...
  color_depth_t Panel_ILI9331::setColorDepth(color_depth_t depth)
  {
  ...
  void Panel_ILI9331::setRotation(uint_fast8_t r)
  {
    r &= 7;
    _rotation = r;
  ...
  void Panel_ILI9331::update_madctl(void)
  {
    if (_bus == nullptr) return;

    startWrite();
    _bus->writeCommand(CMD_ENTRY_MODE << 8, 16);
    uint32_t data = 0;
    ...
    if (_write_bits != 16) data |= 0xC000;
    _bus->writeData(getSwap16(data), 16);
    endWrite();
  }

  void Panel_ILI9331::set_window(uint_fast16_t xs, uint_fast16_t ys, uint_fast16_t xe, uint_fast16_t ye, uint32_t cmd)
  {
    if (_has_align_data)
    {
      _has_align_data = false;
    ...
    if (xs != _xs || xe != _xe)
    {
      ...
      _bus->writeCommand(CMD_H_ADDR2 << 8, 16);
      xs += _colstart;
      _bus->writeData(xs >> 8 | xs << 8, 16);

      _bus->writeCommand(CMD_H_ADDR1 << 8, 16);
      xe += _colstart;
      _bus->writeData(xe >> 8 | xe << 8, 16);

      _bus->writeCommand(0x20 << 8, 16);
      tmp += _colstart;
    ...
    if (ys != _ys || ye != _ye)
    {
      ...
      _bus->writeCommand(CMD_V_ADDR2 << 8, 16);
      ys += _rowstart;
      _bus->writeData(ys >> 8 | ys << 8, 16);

      _bus->writeCommand(CMD_V_ADDR1 << 8, 16);
      ye += _rowstart;
      _bus->writeData(ye >> 8 | ye << 8, 16);

      _bus->writeCommand(0x21 << 8, 16);
      tmp += _rowstart;
      _bus->writeData(tmp >> 8 | tmp << 8, 16);
    }
