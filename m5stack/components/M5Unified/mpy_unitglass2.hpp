...
class MPY_M5UnitGLASS2 : public M5GFX
{
...
public:
    ...
    MPY_M5UnitGLASS2(const config_t &cfg)
    {
    ...
    MPY_M5UnitGLASS2(uint8_t i2c_addr = M5UNITGLASS2_ADDR)
    {
        setup(i2c_addr);
    }

    using lgfx::LGFX_Device::init;
    bool init(uint8_t i2c_addr = M5UNITGLASS2_ADDR)
    {
        setup(i2c_addr);
    ...
    void setup(uint8_t i2c_addr = M5UNITGLASS2_ADDR)
    {
