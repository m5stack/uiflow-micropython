...
class MPY_M5UnitOLED : public M5GFX
{
...
public:
    ...
    MPY_M5UnitOLED(const config_t &cfg)
    {
    ...
    MPY_M5UnitOLED(uint8_t i2c_addr = M5UNITOLED_ADDR)
    {
        setup(i2c_addr);
    }

    using lgfx::LGFX_Device::init;
    bool init(uint8_t i2c_addr = M5UNITOLED_ADDR)
    {
        setup(i2c_addr);
    ...
    void setup(uint8_t i2c_addr = M5UNITOLED_ADDR)
    {
