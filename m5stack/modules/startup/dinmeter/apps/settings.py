...
class ComLinkSetting(AppBase):
    ...
    def on_ready(self):
        ...
        self._menu_label.setText("COM.X Link")
        self._option_img.set_src(self._comlink_options.get(self._option))
...
class GeneralSettingApp(AppBase):
    ...
    async def _keycode_enter_event_handler(self, fw):
        ...
        await app._keycode_enter_event_handler(fw)
...
class SettingsApp(AppBase):
    ...
    async def _keycode_enter_event_handler(self, fw):
        if self._app:
            await self._app._keycode_enter_event_handler(fw)
            return
    ...
    async def _keycode_dpad_down_event_handler(self, fw):
        if self._app:
            await self._app._keycode_dpad_down_event_handler(fw)
            return
    ...
    async def _keycode_dpad_up_event_handler(self, fw):
        if self._app:
            await self._app._keycode_dpad_up_event_handler(fw)
            return
