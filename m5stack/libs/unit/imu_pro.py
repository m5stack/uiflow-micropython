...
#
# SPDX-FileCopyrightText: Copyright (c) 2023 Arduino SA
#
...
SF_G = 1
SF_M_S2 = 9.80665  # Standard gravity, 1 g = 9.80665 m/s²
SF_DEG_S = 1
SF_RAD_S = 57.295779513082  # Radian to degree conversion factor, 1 rad/s = 57.295779578552 deg/s


class IMUProUnit(BMI270_BMM150, BMP280):
    def __init__(
        self,
        i2c: I2C | PAHUBUnit,
        address: int | list | tuple = BMI270_ADDR,
    ) -> None:
        #! Initializes Gyro, Accelerometer and Magnetometer using default values.
        self._i2c = i2c
        if address not in self._i2c.scan():
            raise UnitError("IMU Pro unit maybe not found in Grove")

        BMI270_BMM150.__init__(self, self._i2c)
    ...
    def get_magnetometer(self) -> tuple:
        #! Returns magnetometer vector in uT.
        return self.magnet()

    def get_compass(self) -> float:
        #! Return compass heading value is in range of 0º ~ 360º
    ...
    def get_attitude(self) -> tuple:
        ...
        angleAccX = math.atan2(accY, accZ + abs(accX)) * SF_RAD_S  # noqa: N806
        angleAccY = math.atan2(accX, accZ + abs(accY)) * (-SF_RAD_S)  # noqa: N806
