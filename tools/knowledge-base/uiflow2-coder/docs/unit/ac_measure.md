...
### `class ACMeasureUnit(i2c, address)`
    ...
    - Parameter `i2c`: The I2C interface object or PAHUBUnit used for communication.
    - Parameter `address`: The I2C address of the AC Measure unit, default is 0x42.
...
### `ACMeasureUnit.init_i2c_address(slave_addr)`
    ...
    - Parameter `slave_addr` (`int`): The I2C address of the AC Measure unit, should be between 1 and 127.

### `ACMeasureUnit.get_voltage_str()`

    Get the voltage string value from the AC Measure unit.

    - Returns: The voltage value as a string.
...
### `ACMeasureUnit.get_device_status(mode)`
    ...
    - Parameter `mode` (`int`): The mode to select the desired status (0xFE for firmware version, 0xFF for I2C address).

    - Returns: The requested device status as an integer.

### `ACMeasureUnit.set_i2c_address(addr)`

    Set a new I2C address for the AC Measure unit.

    - Parameter `addr` (`int`): The new I2C address to set, between 0x01 and 0x7F.
