...
### `class FlashLightUnit(port)`
    ...
    - Parameter `The port to which the FlashLightUnit is connected. port[0]` (`port:`): adc pin, port[1]: pump pin.
...
### `FlashLightUnit.flash(brightness: int, time: int, turn_off=False) -> None`
    ...
    - Parameter `brightness`: The brightness of the light.
    ...
    - Parameter `time`: The time of the light.
    ...
    - Parameter `turn_off`: Turn off the light after flash.
