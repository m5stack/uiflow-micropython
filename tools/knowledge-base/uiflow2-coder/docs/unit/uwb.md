...
### `class UWBUnit(id, port, device_mode, device_id, verbose)`
    ...
    - Parameter `id`: UART ID.
    - Parameter `port`: The port that the unit is connected to.
    - Parameter `device_mode`: device mode.
...
### `UWBUnit.get_distance(index)`
    ...
    - Parameter `index` (`int`): anchor ID (0 ~ 3).
...
### `UWBUnit.set_device_mode(mode, id)`
    ...
    - Parameter `mode` (`int`): device mode.
        Options:
        - `Anchor`: UWBUnit.ANCHOR
        - `Tag`: UWBUnit.TAG
    - Parameter `id` (`int`): device ID.
...
### `UWBUnit.set_measurement_interval(interval)`
    ...
    - Parameter `interval` (`int`): measurement interval.

### `UWBUnit.set_measurement(enable)`

    Set the measurement output.

    - Parameter `enable` (`bool`): enable or disable measurement output.

### `UWBUnit.set_callback(anchor, event, callback)`

    Set the callback function for the anchor status.

    - Parameter `anchor` (`int`): anchor ID (0 ~ 3).
    - Parameter `event` (`int`): anchor status.
        Options:
