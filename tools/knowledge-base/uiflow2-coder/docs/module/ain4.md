...
### `class AIN4Module(address)`
    ...
    - Parameter `address` (`intlisttuple`): I2C address of the AIN4Module.
...
### `AIN4Module.get_adc_raw_value(channel) -> int`
    ...
    - Parameter `channel` (`int`): The channel number (1 to 4) to read the ADC value from.
...
### `AIN4Module.get_current_value(channel) -> int`
    ...
    - Parameter `channel` (`int`): The channel number (1 to 4) to read the current value from.
...
### `AIN4Module.set_cal_current(channel, val)`
    ...
    - Parameter `channel` (`int`): The channel number (1 to 4) to set the calibration for.
