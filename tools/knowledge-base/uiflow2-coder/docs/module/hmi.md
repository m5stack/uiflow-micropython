...
### `class HMIModule(address)`
    ...
    - Parameter `address` (`intlisttuple`): I2C address of the HMIModule.

## Methods

### `HMIModule.get_rotary_value() -> int`

    Get the current value of the rotary.

### `HMIModule.set_rotary_value(value)`

    Set the rotary value.

    - Parameter `value` (`int`): rotary value(-2147483648-2147483647).

### `HMIModule.reset_rotary_value()`

    Reset the rotary value.

### `HMIModule.get_rotary_increments() -> int`

    Get the increment value of the rotary.

### `HMIModule.get_button_status(btn_num) -> int`

    Get the state of a specific button.

    - Parameter `btn_num` (`int`): : Button number (1 to 3).

### `HMIModule.get_led_state(led_num) -> int`

    Get the state of a specific LED.

    - Parameter `led_num` (`int`): : LED number (1 to 2).

### `HMIModule.set_led_state(led_num, state)`

    Set the state of a specific LED.

    - Parameter `led_num` (`int`): : LED number (1 to 2).
    - Parameter `state` (`int`): : The state to set for the LED.

### `HMIModule.get_firmware_version() -> int`

    Get the firmware version of the HMI module.

### `HMIModule.get_i2c_address() -> int`

    Get the current I2C address of the HMI module.

### `HMIModule.set_i2c_address(addr)`
