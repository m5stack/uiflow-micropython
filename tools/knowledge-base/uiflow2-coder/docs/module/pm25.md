...
### `PM25Module.set_module_power(state)`
    ...
    - Parameter `state`: Set to True to enable power, False to disable.

### `PM25Module.get_module_power() -> bool`

    Get the PM25Module power state.

    - Parameter `state`: Get the module power state.
...
### `PM25Module.get_pm_data(data_num)`
    ...
    - Parameter `data_num` (`int`): The options range from standard PM concentrations to counts of particles of specific sizes in 0.1 liters of air.
