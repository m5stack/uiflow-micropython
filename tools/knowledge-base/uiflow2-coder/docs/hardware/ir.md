...
### `IR.tx(cmd, data)`
    ...
    - Parameter `cmd`: The command code to be transmitted.
    - Parameter `data`: The data associated with the command.
...
### `IR.rx_cb(cb)`
    ...
    - Parameter `cb`: Callback function with signature `cb(data, addr)`. `data` and `addr` are 8-bit values (0–255).
