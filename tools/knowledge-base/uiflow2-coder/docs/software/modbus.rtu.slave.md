...
### `class ModbusRTUSlave(uart, device_address: int = 1, context=None, ignore_unit_id: bool = False, verbose: bool = False)`
    ...
    - Parameter `uart` (`UART`): UART object or RS485 object.
    - Parameter `device_address` (`int`): Device address. The address is 0 to 247.
    - Parameter `context` (`dict`): Modbus server context.
    - Parameter `ignore_unit_id` (`bool`): Ignore unit ID(or Device address).
...
slave.stop()
```
### `ModbusRTUSlave.add_coil(register: int, value: bool)`

        Add a coil to the modbus register dictionary.

        - Parameter `register` (`int`): address of the coils. The address is 0x0000 to 0xFFFF.
        - Parameter `value` (`bool`): Value to add. The value is True or False.
        - Returns: None

```python
slave.add_coil(0, True)
```
### `ModbusRTUSlave.add_discrete_input(register: int, value: bool)`
...
```
### `ModbusRTUSlave.add_holding_register(register: int, value: int)`
...
```
### `ModbusRTUSlave.add_input_register(register: int, value: int)`
...
```
### `ModbusRTUSlave.remove_coil(register: int)`
...
slave.remove_coil(0)
```
### `ModbusRTUSlave.remove_discrete_input(register: int)`
...
```
### `ModbusRTUSlave.remove_holding_register(register: int)`
...
```
### `ModbusRTUSlave.remove_input_register(register: int)`
...
```
### `ModbusRTUSlave.get_coil(register: int)`
...
```
### `ModbusRTUSlave.get_discrete_input(register: int)`
...
```
### `ModbusRTUSlave.get_holding_register(register: int)`
...
```
### `ModbusRTUSlave.get_input_register(register: int)`
...
```
### `ModbusRTUSlave.set_coil(register: int, value: bool)`

        Set the coil value.

        - Parameter `register` (`int`): address of the coils. The address is 0x0000 to 0xFFFF.
        - Parameter `value` (`bool`): Value to set. The value is True or False.
        - Returns: None

```python
slave.set_coil(0, True)
```
### `ModbusRTUSlave.set_multi_coils(register: int, value: list)`
...
```
### `ModbusRTUSlave.set_discrete_input(register: int, value: bool)`
...
```
### `ModbusRTUSlave.set_multi_discrete_input(register: int, value: list)`
...
```
### `ModbusRTUSlave.set_holding_register(register: int, value: int)`
...
```
### `ModbusRTUSlave.set_multi_holding_register(register: int, value: list)`
...
```
### `ModbusRTUSlave.set_input_register(register: int, value: int)`
...
```
### `ModbusRTUSlave.set_multi_input_register(register: int, value: list)`
...
slave.tick()
```
### `ModbusRTUSlave.set_callback(func_code: int, handler)`

        Set the callback function for the function code.

        - Parameter `func_code` (`int`): Function code. The function code is 1 to 6, 15, 16. the symbol is defined in the modbus.ModbusRTUSlave (\*_EVENT etc.).
