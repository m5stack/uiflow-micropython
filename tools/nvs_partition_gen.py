...
class Page(object):
    ...
    def write_varlen_binary_data(
        self, entry_struct, ns_index, key, data, data_size, total_entry_count, encoding, nvs_obj
    ):
        chunk_start = 0
        chunk_count = 0
    ...
    def write_varlen_data(self, key, data, encoding, ns_index, nvs_obj):
        ...
        if self.version == Page.VERSION2 and (encoding in ["hex2bin", "binary", "base64"]):
            entry_struct = self.write_varlen_binary_data(
                entry_struct, ns_index, key, data, datalen, total_entry_count, encoding, nvs_obj
            )
        else:
...
def generate(args, is_encr_enabled=False, encr_key=None):
    ...
    with open(args.input, "rt", encoding="utf8") as input_file, open(
        args.output, "wb"
        ...
        while True:
            ...
            try:
                ...
                if len(data["key"]) > 15:
                    raise InputError(
                        "Length of key `{}` should be <= 15 characters.".format(data["key"])
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="\nESP NVS partition generation utility",
    ...
    parser_gen = subparser.add_parser(
        "generate", help="Generate NVS partition", formatter_class=argparse.RawTextHelpFormatter
    )
    parser_gen.set_defaults(func=generate)
    parser_gen.add_argument("input", default=None, help="Path to CSV file to parse")
    parser_gen.add_argument("output", default=None, help="Path to output NVS binary file")
    ...
    parser_encr.add_argument("input", default=None, help="Path to CSV file to parse")
    parser_encr.add_argument("output", default=None, help="Path to output NVS binary file")
    ...
    parser_encr.add_argument("--keyfile", default=None, help="Path to output encryption keys file")
    parser_encr.add_argument(
        "--inputkey", default=None, help="File having key for encrypting NVS partition"
    )
    parser_encr.add_argument(
        "--outdir",
        default=os.getcwd(),
    ...
    parser_decr.add_argument(
        "input", default=None, help="Path to encrypted NVS partition file to parse"
    )
    parser_decr.add_argument("key", default=None, help="Path to file having keys for decryption")
    parser_decr.add_argument("output", default=None, help="Path to output decrypted binary file")
    parser_decr.add_argument(
        "--outdir",
        default=os.getcwd(),
        help="Output directory to store files created.\
